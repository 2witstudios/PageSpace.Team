...
import { drives, pages, aiChats, db, and, eq, desc } from '@pagespace/db';
...
export async function POST(request: Request) {
  ...
  try {
    const { title, type, parentId, driveSlug, content } = await request.json();
    ...
    const newPage = await db.transaction(async (tx) => {
      ...
      let aiChat;
      if (type === 'AI_CHAT') {
        [aiChat] = await tx.insert(aiChats).values({ pageId: page.id }).returning();
      }
